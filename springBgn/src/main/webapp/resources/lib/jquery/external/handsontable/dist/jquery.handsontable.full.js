...
Handsontable.Core = function (rootElement, userSettings) {
  ...
  editproxy = { //this public assignment is only temporary
    ...
    init: function () {
      ...
      priv.onPaste = function onPaste(str, event, cellProperties) {
        ...
        var copyFun = function(colum){
			var jsonObj = {};
			var columnId = $(colum).data("columnId");
        	if(groupIdDuplMap[columnId]){
        		jsonObj = groupIdDuplMap[columnId];
			}else{
				$(this).attr('id', 'tmpId_' + String(DateObj.getTime())+Math.round(Math.random()*9999));
				var decodeStr = Base64Encoder.decode($(colum).data('option'));
				jsonObj = $.parseJSON(decodeURIComponent(decodeStr));
				...
				groupIdDuplMap[columnId] = jsonObj;
			}

			
			$(colum).attr('data-option', Base64Encoder.encode(encodeURIComponent(naon.json.getJSONString(jsonObj))));
			$(colum).attr('data-column-id', jsonObj.columnId);
		}
        ...
        for(var r=0; r < inputArray.length; r++){
        	for(var c=0; c < inputArray[r].length; c++){
        		if(inputArray[r][c].indexOf('id') > -1){
            		...
            		$(tmpEl).find('[data-column-id]').each(function(){ copyFun(this)});
  ...
  this.getHtml = function(){
	  ...
	  table.css('font-family',"'맑은 고딕', Malgun Gothic, 'MalgunGothic', 돋움, Dotum, sans-serif");
...
(function(Handsontable){
  ...
  function onAfterChange(changes, source){
    ...
    if(instance && instance.view){
    	instance.$table.imagesLoaded(function(){ //for image
    		instance.view.render();
    	});
    }
  }
