...
(function ($, undefined) {
	...
	function isMail(text) {
		...
		var mailPattern = /[_0-9a-zA-Z-]+[_a-z0-9'`.-]*@[0-9a-zA-Z]([-_\.]?[0-9a-zA-Z])*\.[a-zA-Z]{2,63}/;
		...
		if(text.indexOf("&lt;") != -1) {
			...
			if(info2[0].indexOf(' ') > -1 || !mailPattern.test(info2[0])) return false;
		} else {
			if(text.indexOf(' ') > -1 || !mailPattern.test(text)) return false;
		}
		return true;
	}
	function reservedInputCheck(text){
		var pattern = /^\%.*\%$|^\#.*\#$|^\$.*\$$|^\{.*\}$|^\(.*\)$|^\[.*\]$/;  // 회사명이거나 부서명, 주소록 그룹인 경우 패턴 검사 //직위#, 직책$ 도 추가
		if(text.indexOf("<") != -1) {
	...
	function reservedWorkCheck(text, orgtext){
		...
		if(orgtext.indexOf("<") != -1) {
			orgtext = orgtext.split("<")[0].trim();
		}
		
		// 원본 형식에 따라 input값 체크
		switch(true){
			case cmp.test(orgtext) :
				if(cmp.test(text)) return true;
				break;
			case dept.test(orgtext) :
				if(dept.test(text)) return true;
				break;
			case ofc.test(orgtext) :
				if(ofc.test(text)) return true;
				break;
			case pos.test(orgtext) :
				if(pos.test(text)) return true;
				break;
			case userGroup.test(orgtext) :
				if(userGroup.test(text)) return true;
				break;
			default :
            ...
            if (target) {
                ...
                $(document).find("div.select2-drop-active").each(function () {
                    if (target && this !== target){
                    	...
                    	var select2 = $(document.activeElement).closest('.select2-container').data('select2');
                    	if(select2 && select2.opts.mailCheck){ //전자우편일때 강제로 입력처리
                    		select2.insertTextByForced();
                    	}else {
        ...
        $(document).delegate("body", "blur", function (e) {
        	setTimeout(function(){
        		if(document.activeElement && document.activeElement.nodeName && document.activeElement.nodeName.toLowerCase() === 'iframe'){
        			$(document).find("div.select2-drop-active").each(function () {
        				var select2 = $(this).data("select2");
        				//메일과 같이 자동완성에서 선택하는 것이 아니라 없는 것을 강제로 입력하는 경우에 사용
                    	if(select2.opts.forcedInsert){
                    		select2.insertTextByForced();
    ...
    AbstractSelect2 = clazz(Object, {
        ...
        selectHighlighted: function () {
            ...
            if (data) { // 자동 완성 검색한 값을 선택.
                highlighted.addClass("select2-disabled");
                this.highlight(index);
                this.onSelect(data);
            }else{
            	if(this.opts.ajax && this.opts.ajax.data().typeSearch == 'E'){ // 전자우편
            		this.insertTextByForced();
            	}else{
        ...
        //자동완성 검색 결과와 상관없이 입력한 문자를 강제로 입력(전자우편)
        insertTextByForced: function () {
	    	var input = $.trim($(this.search[0]).val())
	    		, data;
	    	
	    	if(input.length > 1 && $(this.search[0]).val() != this.opts.placeholder){
	    		input = this.opts.tokenizer($(this.search[0]).val(), this.data(), this.bind(this.onSelect), this.opts);
	    		if(!input){
	    	...
	    	if(!reservedInputCheck(input) && input != '' && data) {
				this.onSelect(data);
			}else{
				this.close();
			}
        ...
        initContainerWidth: function () {
            ...
            if (width !== null) {
                ...
                this.selection.find('input.select2-default').attr('style', "width: " + this.container.width() +'px;' );
            }
        },
		setMaximumSelectionSize: function(maxSize) {
			this.opts.maximumSelectionSize = maxSize || 0;
		}
    });
    ...
    MultiSelect2 = clazz(AbstractSelect2, {
        ...
        initContainer: function () {
            ...
            this.search.bind("keydown", this.bind(function (e) {
                ...
                if (e.which === KEY.BACKSPACE && this.search.val() === "") {
                    ...
                    if (choices.length > 0) {
                    	var choiceLast = choices.last();
                    	if(!choiceLast.data("select2-data").indelible){
                    		//대상이 수정/삭제불가 아닌경우에만 포커싱
                    		choiceLast.addClass("select2-search-choice-focus");
                    	}
                    }
            ...
            this.search.bind("blur", this.bind(function(e) {
              ...
              //onblur시에도 입력되게 하기 위해 추가(choidoyoung)
                ...
                setTimeout(function(){
                	if(!self.search.val()) // 검색결과 클릭시, 먼저 동작하여 plcaceHolder가 보이는현상이 있어 값있을 경우에만 실행
                		self.clearSearch();
                }, 0);
                e.stopImmediatePropagation();
            }));
        ...
        addSelectedChoice: function (data) {
            ...
            if(this.opts.mailCheck && data.mailboxType != 'ContactGroup') { //익스체인지 주소록 그룹일 경우 메일체크를 하지않음.
            	...
            	if(data.targetType == '2' || (data.text.indexOf('{') === 0) && data.text.indexOf('}') !== -1) {
                    data.isValid = true;
            	} else {
                    data.isValid = isMail(formatted);
            	}
            	choice.addClass(data.isValid ? "valid" : "invalid" );
            }
            ...
            if(this.opts.editable) {
            	...
            	self.initModifyInput(choice.find("input"), choice.find("div").text());
            	...
            	choice.find("input").bind("focusout keydown", function(evt){
            		if(evt.which == KEY.ENTER) {
            			...
            			if(self.invalidIsMailAndText(editValue, choice.find("div").text())) {
            		...
            		// opts.query가 overwrite 되어 신규 수신자 검색시 ajax 실행안됨.(#122922)
            		if(self.opts.isLoaded){
        				self.opts.query = ajax(self.opts.ajax);
        			}
            	});
            }
        	...
        	if (resize != undefined && !resize) {
        		if(this.opts.width){
    				if(this.opts.width.indexOf('%') > -1){
    					var width = this.container.parent().width() * parseInt(this.opts.width) / 100;
		 ...
		 * @param orgData
         */
        initModifyInput: function (choice, orgData) {
                ...
                if (this.opened()) {
                    ...
                    case KEY.TAB:
                    	if(!e.shiftKey){
    	                    ...
    	                    this.close();
    	                    if (!this.invalidIsMailAndText(data.text, orgData)) {
         ...
         * 잘못된 메일인지 체크 한다.
         */
        invalidIsMailAndText : function (value, orgText) {
        	return !reservedWorkCheck(value.replace("&lt;", "<").replace("&gt;",">"), orgText.replace("&lt;", "<").replace("&gt;",">")) || !isMail(value.replace(/</g, "&lt;").replace(/>/g, "&gt;"))
        }
    });
    ...
    $.fn.select2 = function () {
        ...
        var args = Array.prototype.slice.call(arguments, 0),
            opts,
            select2,
            value, multiple, allowedMethods = ["val", "destroy", "opened", "open", "close", "focus", "isFocused", "container", "onSortStart", "onSortEnd", "onSortStop", "enable", "disable", "positionDropdown", "data","clearSearch","blur","setMaximumSelectionSize"];
        this.each(function () {
