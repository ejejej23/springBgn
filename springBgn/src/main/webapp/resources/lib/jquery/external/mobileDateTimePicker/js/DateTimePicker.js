...
$.DateTimePicker = $.DateTimePicker || {
	...
	i18n: {
		ko : {
			...
			formatHumanDate: function (oDate, sMode, sFormat) {
	            if (sMode === "date")
	                return  oDate.yyyy+"년 " + oDate.month + " " + oDate.dd + "일, " +oDate.dayShort;
	            else if (sMode === "time")
	                return oDate.HH + "시 " + oDate.mm + "분";
	            else if (sMode === "datetime")
	                return oDate.yyyy+"년 " + oDate.month + " " + oDate.dd + "일, " +oDate.dayShort + " " +oDate.HH + "시 " + oDate.mm + "분";
	        }
		},
		...
		vi : {
			...
			titleContentDateTime: "ngày & Thiết lập thời gian",
		
			setButtonContent: "OK",
			clearButtonContent: "Khởi tạo",
			formatHumanDate: function (oDate, sMode, sFormat) {
	            if (sMode === "date")
	                return  oDate.yyyy+"Năm " + oDate.month + " " + oDate.dd + "Ngày, " +oDate.dayShort;
	            else if (sMode === "time")
	                return oDate.HH + "Giờ " + oDate.mm + "phút";
	            else if (sMode === "datetime")
	                return oDate.yyyy+"Năm " + oDate.month + " " + oDate.dd + "Ngày, " +oDate.dayShort + " " +oDate.HH + "Giờ " + oDate.mm + "phút";
	        }
		},
	...
	defaults:  //Plugin Defaults //Defaults는 ko로 셋팅
	{
		mode: "date",
		defaultDate: null,
	...
	DateTimePicker.prototype = {
		...
		_modifyPicker: function()
		{
			...
			for(iTempIndex = 0; iTempIndex < iNumberOfColumns; iTempIndex++)
			{
				...
				if((oDTP.settings.language =="en" && sFieldName == "month") || sFieldName == "meridiem"){
					sDTPickerComp += "<input type='text' class='dtpicker-compValue' "+(oDTP.settings.readonlyInputs ? 'readonly' : '')+">";
				}else{
					sDTPickerComp += "<input type='number' class='dtpicker-compValue' "+(oDTP.settings.readonlyInputs ? 'readonly' : '')+">";
				}
		...
		_addEventHandlersForPicker: function()
		{
			...
			if(!oDTP.settings.readonlyInputs)
			{
				...
				$(".dtpicker-compValue").find("[type='number']").keyup(function() 
				{ 
					this.value = this.value.replace(/[^0-9\.]/g,"");
				});
